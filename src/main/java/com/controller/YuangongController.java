    ...
    public R info(@PathVariable("id") Long id, HttpServletRequest request){
        ...
        if(yuangong !=null){
            ...
            BeanUtils.copyProperties( yuangong , view );//把实体数据重构到view中
                ...
                //级联表
                ShangdianEntity shangdian = shangdianService.selectById(yuangong.getShangdianId());
                if(shangdian != null){
...
//                            yuangongEntity.setShangdianId(Integer.valueOf(data.get(0)));   //商店 要改的
    ...
    public R getCurrYuangong(HttpServletRequest request){
        ...
        if(yuangong !=null){
            ...
            //级联表
                            ShangdianEntity shangdian = shangdianService.selectById(yuangong.getShangdianId());
            if(shangdian != null){
                BeanUtils.copyProperties( shangdian , view ,new String[]{ "id", "createDate"});//把级联的数据添加到view中,并排除id和创建时间字段
                view.setShangdianId(shangdian.getId());
            }
            //修改对应字典表字段
            dictionaryService.dictionaryConvert(view, request);
